...
import pylib.util.img
import pylib.util.plt
import pylib.util.dec
def test_ds_size():
    image_size = (15, 15)
    kernel_size = (2, 2)
    stride = (1, 1)
    np.testing.assert_equal(pylib.util.img.ds_size(image_size, kernel_size, stride), (14, 14))
    
    kernel_size = (2, 2)
    stride = (3, 3)
    np.testing.assert_equal(pylib.util.img.ds_size(image_size, kernel_size, stride), (5, 5))
    
    kernel_size = (3, 3)
    stride = (2, 2)
    np.testing.assert_equal(pylib.util.img.ds_size(image_size, kernel_size, stride), (7, 7))
...
@pylib.util.dec.print_test
def test_blur():
    image_path = '~/dataset/ICDAR2015/Challenge4/ch4_training_images/img_150.jpg'
    image = pylib.util.img.imread(image_path)
    blurred_3 = pylib.util.img.average_blur(image, (3, 3))
    blurred_5 = pylib.util.img.average_blur(image, (5, 5))
    pylib.util.plt.show_images(images = [image, blurred_3, blurred_5], titles = ['origin', '3x3', '5x5'], bgr2rgb = True, share_axis = True)
    
@pylib.til.dec.print_test
def test_rect_perimeter():
    p = pylib.util.img.rect_perimeter((5, 5), (80, 80))
    np.testing.assert_equal(p, 300)
    
    p = pylib.util.img.rect_perimeter((0, 0), (80, 80))
    np.testing.assert_equal(p, 320)

@pylib.util.dec.print_test
def test_rect_perimeter():
    lu, rb = (5, 5), (80, 80)
    p = pylib.util.img.rect_area(lu, rb)
    mask =  pylib.util.img.black((100, 100))
    pylib.util.img.rectangle(mask, lu, rb, border_width = -1, color = 1)
    pylib.util.test.assert_equal(p, np.sum(mask))
    
@pylib.util.dec.print_test
def _test_resize():
    size = np.array((100, 100))
    img = pylib.util.img.black(size)
    pylib.util.img.rectangle(img, (10, 10), (90, 90), color = 1, border_width = -1)
    pylib.util.img.rectangle(img, (20, 20), (80, 80), color = 2, border_width = -1)
    pylib.util.img.rectangle(img, (30, 30), (70, 70), color = 3, border_width = -1)
    pylib.util.img.rectangle(img, (40, 40), (60, 60), color = 4, border_width = -1)

    img1 = pylib.util.img.resize(img, f = 0.5)
    pylib.util.test.assert_equal(util.img.get_shape(img1), 0.5 * size)

    img2 = pylib.util.img.resize(img, fx = 0.1, fy = 1)
    print pylib.util.img.get_wh(img2), (0.1 * size[0], size[1])
    pylib.util.test.assert_equal(pylib.util.img.get_wh(img2), (0.1 * size[0], size[1]))
