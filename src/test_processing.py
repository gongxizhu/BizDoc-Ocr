...
def read_blocks(contour, row_coordinates):
    blocks = []
    inBlock = False
    start = 0
    count_blank = 0
...
def read_text_contours(binary_gray, origin_image):
    text_contours = []
...
def access_pixels(image):
    ...
    binary_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    #ret, binary = cv.threshold(gray, 10, 255, cv.THRESH_BINARY)
    #binary_gray = cv2.medianBlur(binary_gray, 3)
    #binary_gray = cv2.adaptiveThreshold(binary_gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 5, 2)
    #cv2.imwrite(r'C:\my_repo\BizDoc-Ocr\test_1_binary.jpg', binary_gray)
    #binary_gray = cv2.GaussianBlur(binary_gray, (5, 5), 4)
    #ret3, binary_gray = cv2.threshold(binary_gray, 200, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    #ret, binary_gray = cv2.threshold(binary_gray, 50, 255, cv2.THRESH_BINARY)
    #binary_gray = cv2.blur(binary_gray, (5, 5))
    binary_gray = cv2.medianBlur(binary_gray, 3)
    ret, binary_gray = cv2.threshold(binary_gray, 120, 255, cv2.THRESH_BINARY_INV)
    row_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (40, 5))
    row_erosion = cv2.erode(binary_gray, row_kernel, iterations=1)
    row_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (40, 10))
    row_result = cv2.dilate(row_erosion, row_kernel, iterations=1)
    #
    col_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 50))
    col_erosion = cv2.erode(binary_gray, col_kernel, iterations=1)
    col_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (10, 50))
    col_result = cv2.dilate(col_erosion, col_kernel, iterations=1)
    #
    filtered_img = row_result + col_result
    #filtered_img = row_result
    #ret, filtered_img = cv2.threshold(filtered_img, 50, 255, cv2.THRESH_BINARY)
    #filtered_img = cv2.adaptiveThreshold(filtered_img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, 12)
    cv2.imwrite(r'C:\my_repo\BizDoc-Ocr\test_2_binary.jpg', binary_gray)
    # cnts = cv.findContours(binary, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)
    _, contours, hierarchy = cv2.findContours(binary_gray, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(0, len(contours)):
        x, y, w, h = cv2.boundingRect(contours[i])
        area = w * h
    ...
    # #print(len(contours))
    # #cv.drawContours(img, contours, -1, (0, 255, 255), 4)
    # #for i in range(len(cnts) - 1):
    #     # x, y, w, h = cv.boundingRect(cnts[i])
    #     # img[y:(y + h), x:(x + w), :] = black
    #     #cv.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
    cv2.imwrite(r'C:\my_repo\BizDoc-Ocr\test_1_reverse.jpg', image)
    # cv.imshow('pixels_demo', image)


img = cv2.imread(r'C:\my_repo\BizDoc-Ocr\test_2.jpg')
access_pixels(img)
